...
class Solution{
	boolean validate(Node root, int lower,  int upper){
		if(root == null) return true;
		if(root.data<= lower || root.data>=upper) return false;
		boolean leftvalid = validate(root.left,lower,root.data);
		boolean rightvalid = validate(root.right, root.data, upper);
